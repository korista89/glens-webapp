...
function DashboardTab({
  ...
  const averageScore = useMemo(() => (
    domainAverages.length
      ? domainAverages.reduce((sum, entry) => sum + entry.average, 0) / domainAverages.length
      : null
  ), [domainAverages]);
  const topDomain = useMemo(() => {
    if (!domainAverages.length) return null;
    return domainAverages.reduce((best, entry) => (entry.average > best.average ? entry : best), domainAverages[0]);
  }, [domainAverages]);
  const lowDomain = useMemo(() => {
    if (!domainAverages.length) return null;
    return domainAverages.reduce((worst, entry) => (entry.average < worst.average ? entry : worst), domainAverages[0]);
  }, [domainAverages]);
  ...
  const nextGoals = useMemo(() => recommendedGoals.slice(0, 3), [recommendedGoals]);
  const timeline = useMemo(() => buildTimelineRows(evaluationsByMonth), [evaluationsByMonth]);
      ...
      {student ? (
        <div className="dashboard-body">
          ...
          <section className="dashboard-highlight">
            <div className="highlight-card profile-card">
              <div className="highlight-title">학생 요약</div>
              ...
              <div className="profile-score">
                <span>평균 점수</span>
                <strong>{averageScore !== null ? averageScore.toFixed(2) : "-"}</strong>
                <small>최근 평가 기준</small>
              </div>
            </div>
            <div className="highlight-card insight-card">
              <div className="highlight-title">학습 인사이트</div>
              <div className="insight-pills">
                <span>강점 영역: {topDomain ? topDomain.domain.name : "-"}</span>
                <span>집중 영역: {lowDomain ? lowDomain.domain.name : "-"}</span>
                <span>진행 중 목표: {goalState.active.length}</span>
              </div>
              <div className="insight-lists">
                <div>
                  ...
                  <ul>
                    ...
                    )) : <li className="muted">데이터 없음</li>}
                  </ul>
                </div>
                <div>
                  <strong>우선 지원 문항</strong>
                  <ul>
                    {needs.length ? needs.map(entry => (
                      <li key={entry.item.id}>{entry.item.title}</li>
                    )) : <li className="muted">데이터 없음</li>}
            ...
            <div className="highlight-card action-card">
              <div className="highlight-title">추천 다음 목표</div>
              <ul className="next-goals">
                {nextGoals.length ? nextGoals.map(plan => (
                  <li key={`${plan.code}-${plan.levelKey}`}>
                    <div>
                      <strong>{plan.item?.title || plan.goal}</strong>
                      <span className="code-tag">[{plan.code}]</span>
                    </div>
                    <small className="muted">{plan.domain}</small>
                  </li>
                )) : <li className="muted">추천 목표가 없습니다.</li>}
              </ul>
              <div className="next-goals-note">
                <span>목표 목록 탭에서 상세 전략을 확인할 수 있습니다.</span>
              </div>
            </div>
          </section>
